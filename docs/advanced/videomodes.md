The `video_mode` INI setting supports several different methods of specifying the video mode.
...
The VESA [Coordinated Video Timings](https://en.wikipedia.org/wiki/Coordinated_Video_Timings) standard is used to calculate the timing information for the requested video mode. By default the reduced blanking version of the standard is used. This is compatible with most flat panel displays but may not be as well supported on older displays and CRT monitors. You can use regular CVT timings by adding the `cvt` flag to the end of the video mode:

```video_mode=1600,1200,60,cvt ; 1600x1200 on a CRT monitor```

Regular timing requires a slightly higher clock speed for a given resolution, so it some higher resolution modes are not achievable with it. An [online video timing calculator](https://tomverbeure.github.io/video_timings_calculator) can help with understanding the differences between the two methods.
